...
import { delay, shareReplay, takeUntil, tap, withLatestFrom } from 'rxjs/operators';
...
@Component( {
   ...
   template: `
		          <form [formGroup]="form"
		                *ngIf="orders$ | async as extras; else loading">
			          <label>
				          Orders select
				          <select
			          ...
			          <fieldset class="mb-4"
				          ...
				          <p id="extras-errors"
				             aria-live="polite">
          <span *ngIf="form.get('extras')?.errors?.required">
            At least one extra must be selected. Now that's business for you ☺️
          </span>
				          </p>
			          </fieldset>
			          <button appButton
			                  type="submit"
			                  (click)="clickedSubmit$.next()">
				          Submit
			          </button>
		          </form>
